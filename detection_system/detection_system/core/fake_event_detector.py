...
class FakeEventDetector(EventDetector):
    """
    ...
    def __init__(
        self,
        fake_event_factory: Callable = lambda: ViolationPPEEvent(
            cam_id=0,
            persons=[
                PersonInfo(
                    violation=ViolationType.NO_HELMET,
                    bbox_xywh=(0.53, 0.44, 0.04, 0.06),
                )
            ],
        ),
        period: int = 60,
        settings: EventDetectorSettings = settings,
    ):
        self.period = period
        self.fake_event_factory = fake_event_factory
        self.settings = settings
        self.message_sender = PikaMessageSender(
